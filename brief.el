...
;; Wie Sie den brief-mode installieren, sehen Sie auf meiner homepage: http://www.mirkolinkonline.de.
;; Dort gibt es auch die neuesten Versionen zum download.
;; A english version is available, too.

;; Revision: 1.02
;; - meta r korregiert -> fügt jetzt wirklich Dateien ein
;; - buf-list konfigurierbar, ob alle Buffer oder nur Dateien
;; - indent-line auch bei Markierungen
;; - zweiter Parameter für den brief-mode:: wenn nicht nil dann werden Markierungen mit der neuen
...
(defcustom brief-show-complete-bufferlist nil
  "Ist der Wert dieser Variablen ungleich nil, dann werden alle Buffer in der Bufferliste angezeigt."
  :type 'boolean
  :group 'brief)
...
(defvar brief-minibuf-mark-active () "Wenn diese Variable im Minibuffer gesetzt ist, dann wird die Markierung mit der nächsten Cursorbewegung deaktiviert.")
...
(define-key view-mode-map [(control home)]   (lambda ()
					       (interactive)
					       (if (and brief-minibuf-mark-active
...
(defun brief-mouse-set-point (event)
  (interactive "e")
  (setq brief-minibuf-mark-active nil)
  (if (and mark-active
    	   rect-mode-active)
      (brief-switch-column-mark t)
    (if mark-active
...
(define-key brief-mode-map          [(tab)]               'brief-indent-line)
...
(defun brief-indent-line ()
  "Rückt die aktuelle oder die markierten Zeilen ein."
  (interactive)
  (if (and (> (length (buffer-name)) 9)
	       (string= (substring (buffer-name) 0 10) " *Minibuf-"))
      (minibuffer-complete)
    (if mark-active
	(if (not rect-mode-active)
	    (save-excursion
	      (let ((start-zeile (count-lines 1 (region-beginning)))
	   	    (end-zeile (count-lines 1 (region-end))))
	   	(goto-char (region-beginning))
	   	(while (<= (count-lines 1 (point)) end-zeile)
	   	  (beginning-of-line)
...
(defun brief-buf-list ()
  ...
  (interactive)
  (if brief-show-complete-bufferlist
      (bs--show-with-configuration (bs--configuration-name-for-prefix-arg "all"))
    (bs--show-with-configuration (bs--configuration-name-for-prefix-arg nil))))
...
(defun brief-search-forward-run ()
  "Sucht vorwaerts nach der Zeichenkette, die in der Variablen `brief-search-string' gespeichert ist."
...
zu erhalten. Außerdem schaltet diese Funktion die Option truncate-lines ein."
  (if (or (and (> (length mode-name) 9)
	       (string= (substring mode-name 0 10) "Emacs-Lisp"))
...
;;;###autoload
(defun brief-mode (&optional arg delete-selection-mode-active)
...
Graue Taste *		Rückgängig"
  ...
  (when brief-mode
    (when (or delete-selection-mode delete-selection-mode-active)
      (brief-delete-selection-mode))
    (add-hook 'minibuffer-setup-hook 'mark-all-to-point)
...
(defun mark-all-to-point ()
  ""
  (let ((merke-pos (point)))
    (setq brief-minibuf-mark-active t)
    (goto-char 0)
    (set-mark-command ())
    (goto-char merke-pos)))
...
;; delete-selection-mode interface

;;;###autoload
(defalias 'brief-pending-delete-mode 'brief-delete-selection-mode)

;;;###autoload
(defun brief-delete-selection-mode (&optional arg)
...
any selection."
  (interactive "P")
  (setq brief-delete-selection-mode (if arg
				  (> (prefix-numeric-value arg) 0)
				(not brief-delete-selection-mode)))
  (if (not brief-delete-selection-mode)
      (remove-hook 'pre-command-hook 'brief-delete-selection-pre-hook)
    (add-hook 'pre-command-hook 'brief-delete-selection-pre-hook)
    (transient-mark-mode t)))

;;;###autoload
(defcustom brief-delete-selection-mode nil
  "Toggle Delete Selection mode.
See command `brief-delete-selection-mode'.
Setting this variable directly does not take effect;
use either \\[customize] or the function `brief-delete-selection-mode'."
  :set (lambda (symbol value)
	 (brief-delete-selection-mode (or value 0)))
  :initialize 'custom-initialize-default
  :type 'boolean
  :group 'editing-basics
  :require 'brief-delsel)

(defun brief-delete-active-region (&optional killp)
  ;; Zeilen markiert
  (setq brief-minibuf-mark-active nil)
  (if line-mode-active
...
(defun brief-delete-selection-pre-hook ()
  (when (and brief-delete-selection-mode (or (and transient-mark-mode mark-active) line-mode-active rect-mode-active)
	     (not buffer-read-only))
    (let ((type (and (symbolp this-command)
		     (get this-command 'brief-delete-selection))))
      (cond ((eq type 'kill)
	     (brief-delete-active-region t))
	    ((eq type 'yank)
...
(put 'self-insert-command 'brief-delete-selection t)
(put 'self-insert-iso 'brief-delete-selection t)

(put 'yank 'brief-delete-selection 'yank)
(put 'clipboard-yank 'brief-delete-selection 'yank)
(put 'insert-register 'brief-delete-selection t)

(put 'delete-backward-char 'brief-delete-selection 'supersede)
(put 'backward-delete-char-untabify 'brief-delete-selection 'supersede)
(put 'delete-char 'brief-delete-selection 'supersede)

(put 'newline-and-indent 'brief-delete-selection t)
(put 'newline 'brief-delete-selection t)
(put 'open-line 'brief-delete-selection 'kill)

(put 'insert-parentheses 'brief-delete-selection t)

;; This is very useful for cancelling a selection in the minibuffer without
;; aborting the minibuffer.
(defun brief-minibuffer-keyboard-quit ()
  "Abort recursive edit.
In Delete Selection mode, if the mark is active, just deactivate it;
then it takes a second \\[keyboard-quit] to abort the minibuffer."
  (interactive)
  (if (and brief-delete-selection-mode transient-mark-mode mark-active)
...
(when brief-delete-selection-mode
  (brief-delete-selection-mode t))
